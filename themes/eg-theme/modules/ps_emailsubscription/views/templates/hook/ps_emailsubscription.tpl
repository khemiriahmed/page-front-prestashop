...
<div class="block_newsletter col-lg-3 col-12">
  <div class="row">
   <p class="footer__title footer__title--desktop">Rejoignez nous</p>  
    {block name='hook_social'}
        {hook h='displaysocial'}
      {/block}
   <p id="block-newsletter-label" class="p-0 col-lg-12 col-12">{l s='Découvrez nos prochaines recettes en avant première !' d='Shop.Theme.Global'}</p>
    <div class="p-0 col-lg-12 col-12">
      ...
      <form action="{$urls.pages.index}#footer" method="post" class="needs-validation">
        ...
        <div class="clearfix">
            ...
            {/if}
            {if $conditions}
              <p class="small condition-text mt-2">{$conditions}</p>
            {/if}
            {if isset($id_module)}
